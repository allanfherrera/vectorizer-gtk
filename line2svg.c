...
void on_button_clicked(GtkWidget *button, gpointer data) {
    ...
    GtkWidget *dialog = gtk_file_chooser_dialog_new("Select Line Drawing",
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        // Construct conversion commands optimized for line drawings
        char command[2048];
        snprintf(command, sizeof(command), 
                "convert \"%s\" -threshold 50%% -monochrome \"%s\" && "
                "potrace -s -k 0.5 -u 10 \"%s\" -o \"%s\" && "
                "rm \"%s\"",
        ...
        if (result == 0) {
            gtk_label_set_text(GTK_LABEL(widgets->label), "Line drawing conversion successful!");
        } else {
...
int main(int argc, char *argv[]) {
    ...
    gtk_window_set_title(GTK_WINDOW(widgets->window), "Line Drawing to SVG Converter");
    ...
    widgets->label = gtk_label_new("Click to select a line drawing image");
    gtk_box_pack_start(GTK_BOX(vbox), widgets->label, TRUE, TRUE, 0);

    widgets->button = gtk_button_new_with_label("Select Line Drawing");
